...
def test_app_settings_matches_shell_default_values(monkeypatch, tmp_path) -> None:
    ...
    assert settings.proxy_pool_regions == ("tw", "sg", "jp", "hk", "us")
    assert settings.register_fresh_proxy_regions == ("tw", "sg")
    ...
    assert settings.responses_survival_recent_window_seconds == 1800
    assert settings.responses_survival_require_provenance is True
    assert settings.warmup_min_age_seconds == 600
    assert settings.warmup_min_successful_probes == 2
