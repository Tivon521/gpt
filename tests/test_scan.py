...
def test_classify_token_file_uses_browser_fingerprint_headers_for_usage_and_responses(
    monkeypatch,
    tmp_path: Path,
) -> None:
    token_file = tmp_path / "fingerprint@example.com.json"
    token_file.write_text(
        json.dumps({"access_token": "token-1", "account_id": "acct-1"}),
        encoding="utf-8",
    )
    ...
    class Session:
        def get(self, url: str, **kwargs):  # type: ignore[no-untyped-def]
            del url
            calls.append(("GET", dict(kwargs.get("headers") or {})))
            return type("Response", (), {"status_code": 200, "text": '{"ok":true}'})()

        def post(self, url: str, **kwargs):  # type: ignore[no-untyped-def]
            del url
            calls.append(("POST", dict(kwargs.get("headers") or {})))
            return type("Response", (), {"status_code": 200, "text": "data: ok"})()
    ...
    assert result.category == "normal"
    assert [method for method, _headers in calls] == ["GET", "POST"]
    for _method, headers in calls:
        assert headers["User-Agent"] == OPENAI_USER_AGENT
        assert headers["sec-ch-ua"] == OPENAI_SEC_CH_UA
        assert headers["sec-ch-ua-mobile"] == OPENAI_SEC_CH_UA_MOBILE
        assert headers["sec-ch-ua-platform"] == OPENAI_SEC_CH_UA_PLATFORM
