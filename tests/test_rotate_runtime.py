...
def test_runtime_reconcile_ignores_local_warmup_pending_accounts(tmp_path: Path) -> None:
    pool_dir = tmp_path / "pool"
    pool_dir.mkdir()
    local_only = pool_dir / "warmup@example.com.json"
    local_only.write_text(
        json.dumps(
            {
    ...
    assert client.uploads == []
    assert json.loads(local_only.read_text(encoding="utf-8"))["cpa_sync_status"] == "warmup_pending"
